...
A_OUT="$(WCP_AGENT=auth-1 "${CLI[@]}" acquire --path src/a.ts --doing "rotate cookie" --json || true)"
echo "$A_OUT" | grep -q '"ok":true'
B_OUT="$(WCP_AGENT=ui-2 "${CLI[@]}" acquire --path src/a.ts --doing "empty state" --json || true)"
if ! echo "$B_OUT" | grep -q '"error":"conflict"'; then
  echo "expected conflict, got: $B_OUT" >&2
  exit 1
fi
sleep $((TTL + 1))
O_OUT="$(WCP_AGENT=ui-2 "${CLI[@]}" overtake --path src/a.ts --json || true)"
